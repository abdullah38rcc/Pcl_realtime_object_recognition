...
main (int argc, char** argv)
{
  ...
  pcl::PointCloud<PointType>::Ptr complete_scene (
      new pcl::PointCloud<PointType> ());
  ...
  std::vector < pcl::PointCloud < pcl::PointXYZRGB > ::Ptr > model_list =
      ReadModels (argv);
  ...
  Ransac < pcl::SampleConsensusModelSphere
      < pcl::PointXYZRGB >> ransac_estimator;
  ...
  while (!visualizer.viewer_.wasStopped ())
  {
    ...
    if (segment)
      scene = findAndSubtractPlane (scene, segmentation_threshold,
    ...
    else
    {
      ...
      std::cout << "\tfound " << scene_keypoints->points.size ()
          << " keypoints in the scene and " << model_keypoints->points.size ()
      ...
      if (fpfh)
      {
        ...
        KeyDes<pcl::FPFHSignature33,
            pcl::FPFHEstimationOMP<pcl::PointXYZRGB, pcl::Normal,
                pcl::FPFHSignature33> > est (model_list[0], model_keypoints,
            scene, scene_keypoints, model_normals, scene_normals);
        model_scene_corrs = est.run ();
      }
      else if (pfh)
      {
        std::cout << "using pfh descriptors" << std::endl;
        KeyDes<pcl::PFHSignature125,
            pcl::PFHEstimation<pcl::PointXYZRGB, pcl::Normal,
                pcl::PFHSignature125> > est (model_list[0], model_keypoints,
            scene, scene_keypoints, model_normals, scene_normals);
        model_scene_corrs = est.run ();
      }
      else if (pfhrgb)
      {
        ...
        KeyDes<pcl::PFHRGBSignature250,
            pcl::PFHRGBEstimation<pcl::PointXYZRGB, pcl::Normal,
                pcl::PFHRGBSignature250> > est (model_list[0], model_keypoints,
            scene, scene_keypoints, model_normals, scene_normals);
        model_scene_corrs = est.run ();
      }
      else if (ppf)
      {
        ...
        KeyDes<pcl::PPFSignature,
            pcl::PPFEstimation<pcl::PointXYZRGB, pcl::Normal, pcl::PPFSignature> > est (
            model_list[0], model_keypoints, scene, scene_keypoints,
      ...
      else if (ppfrgb)
      {
        ...
        KeyDes<pcl::PPFRGBSignature,
            pcl::PPFRGBEstimation<pcl::PointXYZRGB, pcl::Normal,
                pcl::PPFRGBSignature> > est (model_list[0], model_keypoints,
            scene, scene_keypoints, model_normals, scene_normals);
        model_scene_corrs = est.run ();
      }
      else if (shot)
      {
        ...
        KeyDes<pcl::SHOT352,
            pcl::SHOTEstimationOMP<PointType, NormalType, pcl::SHOT352> > est (
            model_list[0], model_keypoints, scene, scene_keypoints,
            model_normals, scene_normals);
        model_scene_corrs = est.run ();
      }
      ...
      if (use_hough)
      {
        //Hough3D
        Hough hough;
        cluster = hough.GetClusters (model_list[0], model_keypoints,
            model_normals, scene, scene_keypoints, scene_normals,
            model_scene_corrs);
      }
      else
      {
    ...
    std::cout << "\tFound " << std::get < 0
        > (cluster).size () << " model instance/instances " << std::endl;

    SetViewPoint (complete_scene);

    visualizer.Visualize (model_list[0], model_keypoints, complete_scene,
        scene_keypoints, cluster, scene);
  }
  return 0;
}
